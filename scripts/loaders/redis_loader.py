...
class RedisLoader:
    ...
    def __init__(self, host='redis', port=6379, db=0, 
        ...
        # Tester la connexion
        try:
            print(f"🔍 Redis: Tentative de connexion à {host}:{port}")
            self.logger.info(f"🔍 Redis: Tentative de connexion à {host}:{port}")
            self.client.ping()
            print(f"✅ Redis: Connexion établie avec succès")
            self.logger.info("✅ Redis: Connexion établie avec succès")
            self.connected = True
        except (redis.ConnectionError, Exception) as e:
            print(f"❌ Redis: Erreur de connexion - {type(e).__name__}: {e}")
            self.logger.warning(f"❌ Redis: Erreur de connexion - {type(e).__name__}: {e}")
            self.connected = False
