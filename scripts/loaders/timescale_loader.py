...
class TimescaleLoader:
    ...
    def __init__(self):
        ...
        try:
            print(f"🔍 TimescaleDB: Tentative de connexion à {self.host}:{self.port}/{self.database}")
            self.logger.info(f"🔍 TimescaleDB: Tentative de connexion à {self.host}:{self.port}/{self.database}")
            self.create_tables()
            print(f"✅ TimescaleDB: Connexion établie avec succès")
            self.logger.info("✅ TimescaleDB: Connexion établie avec succès")
            self.connected = True
        except Exception as e:
            print(f"❌ TimescaleDB: Erreur de connexion - {type(e).__name__}: {e}")
            self.logger.warning(f"❌ TimescaleDB: Erreur de connexion - {type(e).__name__}: {e}")
            self.connected = False
